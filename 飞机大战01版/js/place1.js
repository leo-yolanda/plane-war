...
function ownAir(option, e) {
    ...
    ownPlane.style.top = oBox.offsetHeight - 70 + "px";
    //整个盒子宽度的一半-飞机宽度的一半
    ownPlane.style.left = oBox.offsetWidth / 2 - 60 + "px";
    //将创建的飞机节点放在盒子里
    oBox.appendChild(ownPlane);

    //我军移动 键盘控制 利用数组记录按下的键
    //飞机最大移动距离
    let maxTop = oBox.offsetHeight - 80;
    let maxLeft = oBox.offsetWidth - 120;
    let arrBool = [];
    document.onkeydown = function(e) {
            //监听上个键按下的同时下个键也能按下触发 鼠标按下的时候同时监听一个状态
            arrBool[e.keyCode] = true;
            //
            arrBool[65] && ownPlane.offsetLeft >= 0 && (ownPlane.style.left =
                ownPlane.offsetLeft - 20 + "px");
            //
            arrBool[87] && ownPlane.offsetTop >= 0 && (ownPlane.style.top =
                ownPlane.offsetTop - 20 + "px");
            //
            arrBool[68] && ownPlane.offsetLeft <= maxLeft && (ownPlane.style.left =
                ownPlane.offsetLeft + 20 + "px");
            //
            arrBool[83] && ownPlane.offsetTop <= maxTop && (ownPlane.style.top =
                ownPlane.offsetTop + 20 + "px");
        }
        //按键松开的时候改变布尔值 按键抬起时状态消失
    document.onkeyup = function(e) {
            arrBool[e.keyCode] = false;
        }
        //*****生成我军子弹
        //根据等级生成子弹间隔
    let arrTime = [500, 400, 300, 100];
    setInterval(() => {
        ...
        biu.style.left = ownPlane.offsetLeft + 3 + "px";
